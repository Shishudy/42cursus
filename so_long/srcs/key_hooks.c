...
/*   Updated: 2023/06/06 17:44:04 by rasantos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	idle_player(t_game *game, int i, int last)
{
	...
	if (last == 119)
		put_image(game, game->map.player.i_u[i], game->map.player.x, game->map.player.y);
	else if (last == 97)
		put_image(game, game->map.player.i_l[i], game->map.player.x, game->map.player.y);
	else if (last == 115)
		put_image(game, game->map.player.i_d[i], game->map.player.x, game->map.player.y);
	else if (last == 100)
		put_image(game, game->map.player.i_r[i], game->map.player.x, game->map.player.y);
}
