...
/*   Updated: 2023/06/18 15:23:50 by rasantos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	clean_images_rest(t_game *game)
{
	...
	mlx_destroy_image(game->mlx_ptr, game->torch[0].img_ptr);
    mlx_destroy_image(game->mlx_ptr, game->torch[1].img_ptr);
    mlx_destroy_image(game->mlx_ptr, game->torch[2].img_ptr);
...
{  
    ...
    {
        put_moving_images(game, game->torch[1], 'T');
        enemy_control(game, n++);
    }
    else if (i == 35000)
    {
        put_moving_images(game, game->torch[2], 'T');
        enemy_control(game, n++);
        i = 0;
    }
    if (n == 6)
